...
public class ChatEndPoint {
    ...
    public void onOpen(Session session,EndpointConfig config){
        ...
        //3、获取所有群组
        String mess2 = MessageUtils.getMessage(true,true,null,messageService.getJoinedGroups(username));
        //4、推送给当前用户
        try{
    ...
    public void onMessage(String message,Session session){
        ...
        try {
            ...
            System.out.println("收到消息：" + mess.toString());
            //获取要将数据发送给的用户
            ...
            if (mess.isGroup()){
                //重新编写消息json串，将isGroup字段设置为true，以便前端js逻辑的判断
                resultMessage = MessageUtils.getMessage(false,true,username,data);
                //获取该群所有的成员，并将当前用户移除（不给自己推送）
                List<String> members = messageService.getAllMembers(toName);
                members.remove(username);
                //是群组消息，那就获取这个群组的所有成员，然后把消息推送给他们
                for (String tempName : members){
                    try {
                        onlineUsers.get(tempName).session.getBasicRemote().sendText(resultMessage);
